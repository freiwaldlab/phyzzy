...
if plotSwitch.meanPSTH % && ~isfield(spikeDataBank, [Some new field generated by function])
  spikeDataBank = meanPSTH(spikeDataBank, meanPSTHParams);
end
...
function spikeDataBank = meanPSTH(spikeDataBank, params)
...
% Concatonating PSTHs.
allStimuliPSTH = zeros(length(allStimuliVec),size(spikeDataBank.(runList{1}).psthByImage{1}{end},2));     % A large matrix for the mean PSTH of all stimuli
...
for stim_ind = 1:length(allStimuliVec)
  ...
  subRunList = runList(find(stimRunIndex));
    ...
    for chan_ind = 1:length(tmpPSTH)
      for unit_ind = 1:length(tmpPSTH{chan_ind})
        ...
        if params.zscorePSTHs
          ...
          fixSD = std(unitActivity(1:abs(spikeDataBank.(subRunList{subRun_ind}).start)));
  ...
  % concatonate collected data into larger, per stimulus array.  
  ...
  if isempty(catagoryPSTHBins{stimIndex(stim_ind)})
    catagoryPSTHBins{stimIndex(stim_ind)} = [catagoryPSTHBins{stimIndex(stim_ind)}; cumulativePSTHAll];
    catagoryPSTHMaxBins{stimIndex(stim_ind)} = [catagoryPSTHMaxBins{stimIndex(stim_ind)}; cumulativePSTHMaxAll];
    catagoryPSTHMaxBinInds{stimIndex(stim_ind)} = [catagoryPSTHMaxBinInds{stimIndex(stim_ind)}; cumulativePSTHMaxIndAll];
  else
    % Once initialized, correct matricies must be stored to the correct
    % group.
    for group_ind = 1:3
      catagoryPSTHBins{stimIndex(stim_ind)}{group_ind} = [catagoryPSTHBins{stimIndex(stim_ind)}{group_ind}; cumulativePSTHAll{group_ind}];
      catagoryPSTHMaxBins{stimIndex(stim_ind)}{group_ind} = [catagoryPSTHMaxBins{stimIndex(stim_ind)}{group_ind}; cumulativePSTHMaxAll{group_ind}];
      catagoryPSTHMaxBinInds{stimIndex(stim_ind)}{group_ind} = [catagoryPSTHMaxBinInds{stimIndex(stim_ind)}{group_ind}; cumulativePSTHMaxIndAll{group_ind}];
    end
  end
end
...
if params.plotTopStim && ~params.broadLabel
  ...
  topIndex = totalStimPresCount > meanPSTHParams.topStimPresThreshold;
...
% For Broad labels, Remove low frequency examples
if params.broadLabel
...
% Take means across all relevant groups and Plot.
groupingType = {'Unsorted','Units','MUA'};
h = gobjects(length(groupingType),3);
...
for group_ind = 1:length(groupingType)
  ...
  for psth_ind = 1:size(catagoryPSTHBins,1)
    ...
    subplot(length(catagoryPSTHMaxBinInds),1, psth_ind);
    roundedMaxBinInds = round(catagoryPSTHMaxBinInds{psth_ind}{group_ind}, -1);
    hist(roundedMaxBinInds, 56); %every 50 ms
    title(sprintf('%s (mode = %s)',uniqueStimLabels{psth_ind}, num2str(mode(roundedMaxBinInds))));
    
    % Overlay a single trace representing all stimuli
    grandSDTrace = [grandSDTrace; SDPSTH(psth_ind,:)];
    grandTrace = [grandTrace; catagoryPSTHBins{psth_ind}{group_ind}];
...
% Count Results of ANOVA across all units.
totalUnitCount = 0;
totalChannelCount = 0;
