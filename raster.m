function [ ] = raster( spikesByItem, pictureLabels, preAlign, postAlign, imDur, ISI, channel_i, unit_i, colors )
...
for item_i = 1:length(spikesByItem)  
  ...
  for trial_i = 1:length(spikesByItem{item_i}{channel_i}{unit_i})
    ...
    for spike_i = 1:length(trialSpikes.times)
      plot([trialSpikes.times(spike_i) trialSpikes.times(spike_i)],[yLevel-0.4 yLevel+0.4],strcat(colors(mod(item_i,length(colors)) + 1),'-'));
    end
  end
  h = plot([0 0],[yLevelStart yLevel],strcat(colors(mod(item_i,length(colors)) + 1),'-'));
