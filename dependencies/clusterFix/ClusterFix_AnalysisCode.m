...
for iter = 1:100
    ...
    SVMstruct = fitcsvm(data(P.training,:),groups(P.training), 'Standardize',true);%,'showplot','true');
    assert(SVMstruct.ConvergenceInfo.Converged, 'SVM training did not reach convergence')
    testresults = predict(SVMstruct,data(P.test,:));%,'showplot','true');
